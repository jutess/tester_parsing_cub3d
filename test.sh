...
SEGFAULT=$(grep "Segmentation fault" fichier | wc -l | sed -e 's/^[ \t]*//')
# ERRORS=$(grep "rror" fichier | wc -l | sed -e 's/^[ \t]*//')
ERRORS=$(grep -c 'rror' fichier | grep -c --before-context=1 'rror')
NBMAPS=$(grep "lecture" fichier | wc -l | sed -e 's/^[ \t]*//')
OPEN=$(($NBMAPS - $SEGFAULT - $ERRORS))
...
# echo -e "${YEL}🍀Number of maps tested👉 : ${RED} $NBMAPS${WHITE}"
# echo -e "${YEL}✅Number of errors detected by your parsing👉 : ${RED} $ERRORS${WHITE}"
# echo -e "${YEL}✅Number of times your program opened and shouldn't👉 : ${RED} $OPEN${WHITE}"
# echo -e "${YEL}💀Number of segfaults👉 : ${RED} $SEGFAULT${WHITE}"
